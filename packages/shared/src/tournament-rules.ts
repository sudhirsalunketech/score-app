 ...
 * for hattrick purposes. A real (non-auto-generated) PENALTY ball — e.g. a tournament penalty
 * unrelated to this rule — is skipped the same way. An auto-generated PENALTY ball (the bonus/
 * penalty runs this same hattrick rule inserts, see scoring.service.ts) is NOT skipped: it still
 * resets the streak, so a longer run of consecutive wickets is only credited once. Any other
 * legal non-wicket delivery resets the streak.
 */
export function computeHattrickSequences(balls: RuleBallInput[], streakLength = 3): Set<number> {
  ...
  for (const ball of balls) {
    ...
    const isNonWicketExtraDelivery =
      (extra === 'WIDE' || extra === 'NO_BALL' || (extra === 'PENALTY' && !ball.isAutoGenerated)) && !ball.isWicket;
 ...
 * of the other value breaks it, same as a dot ball would). Wides/no-balls are skipped — they
 * aren't fair deliveries — and so is a real (non-auto-generated) PENALTY ball. An auto-generated
 * PENALTY ball (the bonus this same streak rule inserts) is NOT skipped, so a longer run of
 * qualifying boundaries is only credited once. Any other legal ball that isn't a qualifying
 * boundary (dot, single, bye, wicket, a mismatched 4/6, etc.), or a boundary in a different over
 * than the streak so far, resets it.
 */
export function computeBoundaryStreakSequences(balls: RuleBallInput[], opts?: { streakLength?: number; runValue?: 4 | 6 }): Set<number> {
  ...
  for (const ball of balls) {
    const extra = ball.extraType ?? 'NONE';
    if (extra === 'WIDE' || extra === 'NO_BALL' || (extra === 'PENALTY' && !ball.isAutoGenerated)) continue;
