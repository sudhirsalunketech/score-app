...
export function validatePlayingXi(input: {
  teamId: string;
  ...
  if (ids.length !== required) {
    errors.push({ code: 'COUNT', message: `Playing XI must have exactly ${required} players` });
  }
  ...
  if (captains.length !== 1) errors.push({ code: 'CAPTAIN', message: 'Select exactly one captain from the Playing XI' });
  if (vices.length !== 1) errors.push({ code: 'VICE_CAPTAIN', message: 'Select exactly one vice captain from the Playing XI' });
  if (keepers.length !== 1) errors.push({ code: 'WICKET_KEEPER', message: 'Select exactly one wicket keeper from the Playing XI' });
